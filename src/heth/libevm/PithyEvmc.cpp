...
std::string getInstrName(const uint8_t instr)
{
    return instructionInfo((Instruction)instr).name;
}
...
evmc::result PithyEVMC::exec(evmc::Host& host, evmc_revision rev, const bool fCreate, const uint64_t gas, const evmc::address& destination, const evmc::address& sender,
                             const evmc::uint256be& value, const bytes& data, const bytes& code, evmc_operation_trace_fn trace_fn, void* trace_user)
{
    evmc_message msg = {
        .kind = EVMC_CALL,
        .flags = 0,
    ...
    return execute(host, rev, msg, code.data(), code.size(), trace_fn, trace_user);
}
