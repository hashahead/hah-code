 ...
 *
 * This callback function is used by a VM to query the given account transient storage entry.
 *
 * @param context  The Host execution context.
 * @param address  The address of the account.
 * @param key      The index of the account's transient storage entry.
 * @return         The storage value at the given transient storage key or null bytes
 *                 if the account does not exist.
 */
 ...
 *
 * This callback function is used by a VM to update the given account transient storage entry.
 * The VM MUST make sure that the account exists. This requirement is only a formality because
 * VM implementations only modify transient storage of the account of the current execution context
 * (i.e. referenced by evmc_message::destination).
 *
 * @param context  The pointer to the Host execution context.
 * @param address  The address of the account.
 ...
 * @return         The effect on the storage item.
 */
typedef enum evmc_storage_status (*evmc_set_transient_storage_fn)(struct evmc_host_context* context,
                                                        const evmc_address* address,
                                                        const evmc_bytes32* key,
                                                        const evmc_bytes32* value);
...
enum evmc_revision
{
     ...
     * EIP-1153 "Transient Storage" (TLOAD, TSTORE opcode)
...
/**
 * This function is the instruction execution log for callback tracking.
 *
 * @param steps       What are the steps of instruction execution.
 * @param pc          program counter.
 * @param instr       current operation.
 * @param newMemSize  new memory size.
 * @param gasCost     operation gas.
 * @param gas         left gas.
 * @param stack       stack pointer.
 * @param stack_count stack count.
 * @param mem         memory pointer.
 * @param mem_count   memory count.
 * @param user        user pointer.
 */
typedef void (*evmc_operation_trace_fn)(const uint64_t steps,
                                        const uint64_t pc,
                                        const uint8_t instr,
 ...
 * @param trace_fn   This function is the instruction execution log for callback tracking.
 * @param trace_user Trace user data.
...
typedef struct evmc_result (*evmc_execute_fn)(struct evmc_vm* vm,
                                              ...
                                              uint8_t const* code,
                                              size_t code_size,
                                              evmc_operation_trace_fn trace_fn,
                                              void* trace_user);
