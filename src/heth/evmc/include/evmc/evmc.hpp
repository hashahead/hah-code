...
public:
    ...
    virtual bytes32 get_transient_storage(const address& addr, const bytes32& key) const
        noexcept = 0;

    /// @copydoc evmc_host_interface::set_transient_storage
    virtual evmc_storage_status set_transient_storage(const address& addr,
                                                      const bytes32& key,
                                                      const bytes32& value) noexcept = 0;
...
public:
    ...
    evmc_storage_status set_transient_storage(const address& address,
                                              const bytes32& key,
                                              const bytes32& value) noexcept final
    {
        return host->set_transient_storage(context, &address, &key, &value);
    }
...
inline evmc_storage_status set_transient_storage(evmc_host_context* h,
                                                 const evmc_address* addr,
                                                 const evmc_bytes32* key,
                                                 const evmc_bytes32* value) noexcept
{
    return Host::from_context(h)->set_transient_storage(*addr, *key, *value);
}
...
{
    static constexpr evmc_host_interface interface{::evmc::internal::account_exists,
