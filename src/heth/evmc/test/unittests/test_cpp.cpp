...
public:
    ...
    evmc::bytes32 get_transient_storage(const evmc::address&, const evmc::bytes32&) const
        noexcept final
    {
        return {};
    }

    evmc_storage_status set_transient_storage(const evmc::address&,
                                              const evmc::bytes32&,
                                              const evmc::bytes32&) noexcept final
    {
        return {};
...
TEST(cpp, vm)
{
    ...
    auto res = vm.execute(host, nullptr, EVMC_MAX_REVISION, {}, nullptr, 0, nullptr, nullptr);
...
TEST(cpp, vm_execute_precompiles)
{
    ...
    auto res = vm.execute(EVMC_MAX_REVISION, msg, nullptr, 0, nullptr, nullptr);
...
TEST(cpp, vm_execute_with_null_host)
{
    ...
    auto vm = evmc::VM{evmc_create_example_vm()};
    evmc_message msg{};
    auto res = vm.execute(host, EVMC_MAX_REVISION, msg, nullptr, 0, nullptr, nullptr);
    EXPECT_EQ(res.status_code, EVMC_FAILURE);
}
