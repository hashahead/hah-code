...
bool VM::initMetrics()
{
    for (std::size_t revision = EVMC_ISTANBUL; revision <= EVMC_MAX_REVISION; ++revision)
    {
        auto& metrics = s_metrics[revision];

        // Copy the metrics of the given EVM revision.
        auto const metricsTable = evmc_get_instruction_metrics_table(static_cast<evmc_revision>(revision));
        if (metricsTable)
        {
...
void VM::optimize()
{
    ...
    for (size_t pc = 0; pc < nBytes; ++pc)
    {
        ...
        if (
            op == Instruction::PUSHC || op == Instruction::JUMPC || op == Instruction::JUMPCI)
        {
            TRACE_OP(1, pc, op);
        ...
        if (op == Instruction::JUMPDEST)
        {
            ...
            (byte)Instruction::PUSH1 <= (byte)op && (byte)op <= (byte)Instruction::PUSH32)
        {
