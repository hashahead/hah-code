...
evmc_result execute(evmc_vm* _instance, const evmc_host_interface* _host,
    ...
    size_t _codeSize, evmc_operation_trace_fn trace_fn, void* trace_user) noexcept
{
    (void)_instance;
    std::unique_ptr<dev::eth::VM> vm{new dev::eth::VM(trace_fn, trace_user)};
...
{
    if (m_onOp){
        evmc_uint256be* stack = nullptr;
        size_t stack_count = m_stackEnd - m_SP;
        if (stack_count > 0)
        {
            stack = new evmc_uint256be[stack_count];
            
            size_t i = 0;
            intx::uint256* p = m_stackEnd - 1;
            while (p >= m_SP && i < stack_count){
                stack[i++] = intx::be::store<evmc_uint256be>(*p);
                p--;
            }
        }

        evmc_uint256be* mem = nullptr;
        size_t mem_count = m_mem.size() / 32;
        if (mem_count > 0)
        {
            mem = new evmc_uint256be[mem_count];

            for (size_t i = 0; i < mem_count; i++)
            {
                memcpy(&(mem[i].bytes[0]), &(m_mem[i*32]), 32);
            }
        }
...
void VM::interpretCases()
{
    ...
    DO_CASES
    {
        ...
        CASE(TLOAD)
        {
            ...
            m_runGas = VMSchedule::tstoreGas;
            updateIOGas();

            auto const key = intx::be::store<evmc_uint256be>(m_SP[0]);
            m_SPP[0] =
                intx::be::load<intx::uint256>(m_host->get_transient_storage(m_context, &m_message->destination, &key));
        }
        NEXT
        ...
        CASE(TSTORE)
        {
            ...
            m_host->set_transient_storage(m_context, &m_message->destination, &key, &value);
        }
        NEXT
        ...
        CASE(MCOPY)
        {
            ...
            const uint64_t dst = toInt63(m_SP[0]);
            const uint64_t src = toInt63(m_SP[1]);
            const uint64_t len = toInt63(m_SP[2]);

            const uint64_t endOfAccess = src + len;
            if (m_mem.size() < endOfAccess)
                throwBufferOverrun(endOfAccess);

            m_copyMemSize = len;
            updateMem(memNeed(dst, len));
            updateIOGas();

            if (len > 0)
            {
                bytes tempBuf(m_mem.data()+src, m_mem.data()+src+len);
                memcpy(m_mem.data()+dst, tempBuf.data(), len);
            }
        }
        NEXT
