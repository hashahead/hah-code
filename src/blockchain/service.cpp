...
bool CService::RetrieveDestState(const uint256& hashFork, const uint256& hashBlock, const CDestination& dest, CDestState& state)
{
    return pBlockChain->RetrieveDestState(hashFork, hashBlock, dest, state);
}
...
bool CService::RetrieveTxContractReceipt(const uint256& hashFork, const uint256& txid, TxContractReceipts& tcrReceipt)
{
    return pBlockChain->RetrieveTxContractReceipt(hashFork, txid, tcrReceipt);
}

bool CService::ListBlockContractReceipt(const uint256& hashFork, const uint256& hashBlock, BlockContractReceipts& vContractReceipts)
{
...
bool CService::ListBlockContractPrevState(const uint256& hashFork, const uint256& hashBlock, BlockContractPrevState& vBlockContractPrevState)
{
...
bool CService::SetContractTransaction(const uint256& hashFork, const uint256& hashLastBlock, const bytes& btFormatData, const bytes& btContractCode, const bytes& btContractParam, CTransaction& txNew, std::string& strErr)
{
    ...
    if (txNew.GetToAddress().IsNull())
    {
        ...
        else
        {
            ...
            {
                uint256 hashContractCreateCode = hashahead::crypto::CryptoKeccakHash(btContractCode.data(), btContractCode.size());
