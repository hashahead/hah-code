...
CTransactionData TxToJSON(const uint256& txid, const CTransaction& tx, const uint256& hashFork, const uint256& blockHash, const int nDepth)
{
    ...
    ret.strData = hnbase::ToHexString(tx.GetTxExtData());
    ...
    ret.strSig = hnbase::ToHexString(tx.GetSignData());
  ...
  : IIOModule("rpcmod")
{
    ...
    std::map<std::string, RPCFunc> temp_map = boost::assign::map_list_of
        ...
        //
        ("getpledgevotes", &CRPCMod::RPCGetPledgeVotes)
        //
        ("listpledgevotes", &CRPCMod::RPCListPledgeVotes)
        //
...
CRPCResultPtr CRPCMod::RPCStop(const CReqContext& ctxReq, CRPCParamPtr param)
{
    ...
    pService->Stop();
    return MakeCStopResultPtr("hashahead server stopping");
}

CRPCResultPtr CRPCMod::RPCVersion(const CReqContext& ctxReq, CRPCParamPtr param)
{
    string strVersion = string("HashAhead server version is v") + VERSION_STR + string(", git commit id is ") + GetGitVersion();
...
CRPCResultPtr CRPCMod::RPCGetPledgeVotes(const CReqContext& ctxReq, CRPCParamPtr param)
{
    auto spParam = CastParamPtr<CGetPledgeVotesParam>(param);
    ...
    auto spResult = MakeCGetPledgeVotesResultPtr();

    spResult->strVoteaddress = destVote.ToString();
    spResult->strDelegateaddress = ctxPledgeVote.destDelegate.ToString();
    spResult->strOwneraddress = ctxPledgeVote.destOwner.ToString();
    spResult->nPledgetype = ctxPledgeVote.nPledgeType;
    spResult->nCycles = ctxPledgeVote.nCycles;
    spResult->nNonce = ctxPledgeVote.nNonce;
    spResult->strVoteamount = CoinToTokenBigFloat(ctxPledgeVote.nVoteAmount);
    spResult->nVotetime = ctxPledgeVote.nVoteTime;
    spResult->nVoteheight = ctxPledgeVote.nVoteHeight;
    spResult->nStopheight = ctxPledgeVote.nStopHeight;
    spResult->nRedeemheight = ctxPledgeVote.nRedeemHeight;
    spResult->nStatus = ctxPledgeVote.nStatus;
    spResult->strRevoterewardamount = CoinToTokenBigFloat(ctxPledgeVote.nReVoteRewardAmount);
    spResult->strStopedrewardamount = CoinToTokenBigFloat(ctxPledgeVote.nStopedRewardAmount);
    spResult->strTotalrewardamount = CoinToTokenBigFloat(ctxPledgeVote.nTotalRewardAmount);
...
CRPCResultPtr CRPCMod::RPCSignMessage(const CReqContext& ctxReq, CRPCParamPtr param)
{
    ...
    else
    {
        ...
        {
            CBufStream ss;
            const string strMessageMagic = "HashAhead Signed Message:\n";
            ss << strMessageMagic;
            ss << strMessage;
...
CRPCResultPtr CRPCMod::RPCFuncSign(const CReqContext& ctxReq, CRPCParamPtr param)
{
    ...
    return MakeCFuncSignResultPtr(hnbase::ToHexString(ret));
}
...
CRPCResultPtr CRPCMod::RPCEthGetBlockTransactionCountByNumber(const CReqContext& ctxReq, CRPCParamPtr param)
{
    ...
    return MakeCeth_getBlockTransactionCountByNumberResultPtr(ToHexString((status.nUserTxCount + status.nRewardTxCount) - (statusPrev.nUserTxCount + statusPrev.nRewardTxCount)));
}
...
CRPCResultPtr CRPCMod::RPCEthDebugTraceBlock(const CReqContext& ctxReq, CRPCParamPtr param)
{
    if (!BasicConfig()->fTraceDb)
    {
        throw CRPCException(RPC_INVALID_REQUEST, "If you need this function, please set config 'tracedb=true', clear data, and then restart");
    ...
    if (strBlockData.empty())
    {
        throw CRPCException(RPC_INVALID_PARAMETER, "Invalid block encode");
    }
    bytes btBlockData = ParseHexString(strBlockData);
    ...
    if (block.GetChainId() != CBlock::GetBlockChainIdByHash(ctxReq.hashFork))
    {
        throw CRPCException(RPC_INVALID_PARAMETER, "Invalid chainid");
    }
    uint256 hashBlock = block.GetHash();
    ...
    auto spResult = MakeCEthDebugTraceBlockResultPtr();
...
CRPCResultPtr CRPCMod::RPCEthDebugTraceBlockByHash(const CReqContext& ctxReq, CRPCParamPtr param)
{
    if (!BasicConfig()->fTraceDb)
    {
        throw CRPCException(RPC_INVALID_REQUEST, "If you need this function, please set config 'tracedb=true', clear data, and then restart");
    ...
    if (strBlockHash.empty())
    {
        throw CRPCException(RPC_INVALID_PARAMETER, "Invalid block");
    }
    uint256 hashBlock = GetRefBlock(ctxReq.hashFork, strBlockHash);
