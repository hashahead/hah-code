...
bool CBbEntry::Initialize(int argc, char* argv[])
{
    ...
    {
        cout << "HashAhead version is v" << VERSION_STR << ", git commit id is " << GetGitVersion() << endl;
        return false;
    }
...
#if !defined(WIN32) && !defined(__APPLE__)
    StdLog("HashAhead", "malloc_trim: %d.", malloc_trim(0));
#endif
...
bool CBbEntry::InitializeModules(const EModeType& mode)
{
    ...
    for (auto& m : modules)
    {
        ...
        case EModuleType::RPCMODE:
        {
            ...
            std::vector<hnbase::CHttpHostConfig> vHostCfg;
...
bool CBbEntry::GetRPCHostConfig(std::vector<hnbase::CHttpHostConfig>& vHostCfg)
{
...
{
    // Windows: ./.hashahead or C:\.hashahead
    // Mac: ~/Library/Application Support/HashAhead
    // Unix: ~/.hashahead
    ...
    //{
    //    return path(pszPath) / "HashAhead";
    //}
    char programPath[MAX_PATH] = { 0 };
    if (GetModuleFileName(NULL, programPath, MAX_PATH) > 0)
    {
        return path(programPath).parent_path() / ".hashahead";
    }
    else
    {
        return path("C:/.hashahead");
    }
#else
    path pathRet;
...
bool CBbEntry::RunInBackground(const path& pathData)
{
    ...
    if (pid > 0)
    {
        FILE* file = fopen((pathData / "hashahead.pid").string().c_str(), "w");
        if (file)
        {
            fprintf(file, "%d\n", pid);
            fclose(file);
...
{
#ifndef _WIN32
    boost::filesystem::remove(pathData / "hashahead.pid");
#endif
}
