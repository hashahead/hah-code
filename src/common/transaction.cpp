...
bool CTransaction::SetTxExtData(const bytes& btExtData)
{
    ...
    try
    {
        hnbase::CBufStream ss(btExtData);
        ss >> mapTxData;
    }
...
bool CTransaction::GetCreateCodeContext(uint8& nCodeType, CTemplateContext& ctxTemplate, CTxContractData& ctxContract) const
{
    ...
    try
    {
        hnbase::CBufStream ss(btCodeData);
        ss >> nCodeType;
...
//-------------------------------------
void CTransaction::Serialize(hnbase::CStream& s, hnbase::SaveType&) const
{
...
void CTransaction::Serialize(hnbase::CStream& s, hnbase::LoadType&)
{
    s >> nType;
    if (nType == TX_ETH_CREATE_CONTRACT || nType == TX_ETH_MESSAGE_CALL)
    {
...
void CTransaction::Serialize(hnbase::CStream& s, std::size_t& serSize) const
{
    (void)s;
    hnbase::CBufStream ss;
    if (nType == TX_ETH_CREATE_CONTRACT || nType == TX_ETH_MESSAGE_CALL)
...
uint256 CTxContractData::GetSourceCodeHash() const
{
    ...
    if (nCompressSource != 0)
    {
        ...
        return hashahead::crypto::CryptoKeccakHash(btDst.data(), btDst.size());
    }
    return hashahead::crypto::CryptoKeccakHash(btSource.data(), btSource.size());
}
...
uint256 CTxContractData::GetContractCreateCodeHash() const
{
    ...
    else
    {
        if (nCompressCode != 0)
        {
            ...
            //return hashahead::crypto::CryptoHash(btDst.data(), btDst.size());
            return hashahead::crypto::CryptoKeccakHash(btDst.data(), btDst.size());
        }
        //return hashahead::crypto::CryptoHash(btCode.data(), btCode.size());
        return hashahead::crypto::CryptoKeccakHash(btCode.data(), btCode.size());
    }
}
