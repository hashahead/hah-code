...
interface voteInterface {
    ...
    function getDelegateTemplateAddress(address mintAddress, address ownerAddress, uint16 rewardRatio) external view returns (address);
    function getDelegateTemplateParam(address delegateAddress) external view returns (address, address, uint16);
    ...
    function getPledgeTemplateAddress(address delegateAddress, address ownerAddress, uint32 nPledgeType, uint32 nCycles, uint32 nNonce) external view returns (address);
    function getPledgeTemplateParam(address pledgeAddress) external view returns (address, address, uint32, uint32, uint32);
...
interface timeVaultWhitelistInterface {
    function addTimeVaultWhitelist(address addrWhitelist) external returns (bool);
    function listTimeVaultWhitelist() external view returns (address[] memory);
    function getTimeVaultWhitelistCount() external view returns (uint);
}

interface forkInterface {
    function stopFork(uint32 nChainId) external returns (bool);
    function redeemForkPledgeBalance(uint32 nChainId) external returns (bool);
    function getForkPledgeBalance(uint32 nChainId) external view returns (uint nBalance, uint nLockedAmount, uint nAvailAmount);
}
